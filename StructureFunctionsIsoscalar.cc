...
int main()
{
  ...
  const auto PDFs = [&] (double const& x, double const& Q) -> std::map<int, double>
    {
      // Construct PDF for isoscalar target
      const std::map<int, double> ppdf = dist->xfxQ(x, Q);
      std::map<int, double> dpdf = ppdf;
      dpdf[1]  = ( ppdf.at(1) + ppdf.at(2) ) / 2;
      dpdf[2]  = dpdf[1];
      dpdf[-1] = ( ppdf.at(-1) + ppdf.at(-2) ) / 2;
      dpdf[-2] = dpdf[-1];
      return apfel::PhysToQCDEv(dpdf);
    };
